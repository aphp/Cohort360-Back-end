...
class CSVExporter(BaseExporter):
    ...
    def get_source_cohorts(export_data: dict, **kwargs) -> List[str]:
        source_cohorts_ids = [t.get("cohort_result_source") for t in export_data.get("export_tables", []) if t.get("cohort_result_source")]
        ...
        if not CohortResult.objects.filter(Q(pk=source_cohort_id) & Q(owner=kwargs.get("owner"))).exists():
    ...
    def handle_export(self, export: Export, params: dict = None) -> None:
        ...
        params = params or {"joinOnPrimaryKey": export.group_tables, "output": {"type": self.type, "filePath": f"{export.target_full_path}.zip"}}
        ...
        pivot_merge_2 = []
        for t in export.export_tables.filter(Q(pivot_merge_columns__isnull=False) | Q(pivot_merge_ids__isnull=False)):
            d = {"tableName": t.name}
