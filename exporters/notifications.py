...
def export_received(action: str, export_type: str, **kwargs):
    ...
    return {
        "subject": f"[Cohorte {kwargs.get('cohort_id')}] {action}",
...
def export_failed_notif_for_owner(**kwargs):
    ...
    context = {**BASE_CONTEXT, "recipient_name": kwargs.get("recipient_name"), "cohort_id": kwargs.get("cohort_id")}
    return {
        "subject": subject,
        "to": [kwargs.get("recipient_email")],
...
def export_failed_notif_for_admins(**kwargs):
    context = {**BASE_CONTEXT, **kwargs, "recipient_name": "Admin"}
    return {
        "subject": "Demande d'export échouée",
...
def csv_export_received(**kwargs):
    return export_received(action=f"Demande d'export `{kwargs.get('cohort_name', CSV)}` reçue", export_type=CSV, **kwargs)


def hive_export_received(**kwargs):
    return export_received(action="Demande reçue de transfert en environnement Jupyter", export_type=HIVE, **kwargs)
