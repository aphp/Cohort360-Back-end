...
class ContentViewSetTest(TestCaseWithDBs):
    def setUp(self):
        ...
        self.valid_content_data = {"title": "Test Content", "content": "Test Content Body", "page": "test-page", "content_type": "BANNER_INFO"}

        self.content_data_with_metadata = {**self.valid_content_data, "metadata": {MetadataName.ORDER.value: "1", MetadataName.STYLE.value: "dark"}}
    ...
    def test_create_content(self):
        new_content_data = {"title": "New Content", "content": "New Content Body", "page": "new-page", "content_type": "BANNER_WARNING"}
    ...
    def test_update_content(self):
        updated_data = {**self.valid_content_data, "title": "Updated Title"}
    ...
    def test_partial_update_content(self):
        response = self.admin_client.patch(self.detail_url, {"title": "Partially Updated Title"}, format="json")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data["title"], "Partially Updated Title")

    def test_unauthorized_partial_update_content(self):
        response = self.client.patch(self.detail_url, {"title": "Partially Updated Title"}, format="json")
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)

    def test_update_content_without_changing_metadata(self):
        # First create content with metadata
        response = self.admin_client.post(reverse("webcontent:contents-list"), self.content_data_with_metadata, format="json")
        content_id = response.data["id"]
        original_metadata = response.data["metadata"]

        # Update only content fields
        update_data = {"title": "Updated Title", "content": "Updated Content Body", "page": "test-page", "content_type": "BANNER_INFO"}
        response = self.admin_client.patch(reverse("webcontent:contents-detail", kwargs={"pk": content_id}), update_data, format="json")
    ...
    def test_update_content_clearing_metadata(self):
        ...
        update_data = {**self.valid_content_data, "metadata": {}}
        response = self.admin_client.patch(reverse("webcontent:contents-detail", kwargs={"pk": content_id}), update_data, format="json")
        self.assertEqual(response.status_code, 200)
    ...
    def test_filter_by_content_type(self):
        Content.objects.create(title="Blog Post", content="Blog Content", page="blog", content_type="BANNER_ERROR")
...
class UnauthenticatedContentViewSetTest(TestCaseWithDBs):
    def setUp(self):
        self.client = APIClient()
        self.valid_content_data = {"title": "Test Content", "content": "Test Content Body", "page": "test-page", "content_type": "BANNER_ERROR"}
        self.list_url = reverse("webcontent:contents-list")
