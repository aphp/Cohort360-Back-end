...
class ContentSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Content
        fields = ["id", "title", "content", "page", "content_type", "content_type_display", "metadata", "created_at", "modified_at", "deleted_at"]
    ...
    def validate_metadata(self, value):
        ...
        if invalid_names:
            raise serializers.ValidationError(f"Invalid metadata name(s): {', '.join(invalid_names)}. Must be one of {', '.join(valid_names)}")
        return value

    def _metadata_dict_to_list(self, metadata_dict):
        """Convert metadata from dict format to list of name-value pairs"""
        return [{"name": name, "value": str(value)} for name, value in metadata_dict.items()]

    def _metadata_list_to_dict(self, metadata_list):
        """Convert metadata from list of name-value pairs to dict format"""
        return {item.name: item.value for item in metadata_list}
    ...
    def validate_content_type(self, value):
        if value not in ContentManagementConfig.CONTENT_TYPES:
            raise serializers.ValidationError(f"Invalid content type. Available types are: {', '.join(ContentManagementConfig.CONTENT_TYPES.keys())}")
        return value
