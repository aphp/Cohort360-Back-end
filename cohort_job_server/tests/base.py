...
class BaseTest(TestCase):
    def setUp(self):
        ...
        self.request = Request.objects.create(owner=self.user, name="Request 01", parent_folder=self.folder)
        self.snapshot = RequestQuerySnapshot.objects.create(owner=self.user, request=self.request, serialized_query=self.json_query)
