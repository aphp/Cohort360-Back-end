...
class AuthenticatedApplicativeUserPermission(IsAuthenticated):
    def has_permission(self, request, view):
        return super().has_permission(request, view) and request.user.username in APPLICATIVE_USERS


class QueryExecutororETLCallbackPermission(AuthenticatedApplicativeUserPermission):
    def has_permission(self, request, view):
        return super().has_permission(request, view) and request.method in ("GET", "PATCH")
