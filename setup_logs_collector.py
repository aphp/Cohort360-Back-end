...
def configure_handlers() -> List[logging.Handler]:
    ...
    formatter = JsonFormatter(
        "%(name)s - %(filename)s - %(message)s - %(asctime)s - %(process)s - %(trace_id)s - %(user_id)s - %(impersonating)s - %(levelname)s",
        rename_fields={
            "asctime": "timestamp",
            "trace_id": "x_traceId",
            "user_id": "x_userId",
...
def main():
    logging.basicConfig(level=logging.INFO, handlers=configure_handlers())
