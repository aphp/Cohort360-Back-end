...
def find_related_codes(codes: str):
    ...
    return ",".join(
        [
            find_mapped_code(
                code, SRC_CODESYSTEM, TARGET_CODEYSTEM, lambda c: TARGET_CODEYSTEM + "|NON RENSEIGNE", connections["omop"], code_mapping_cache
            )
...
FILTER_VALUE_MAPPING = {"Condition": {"code": {MATCH_ALL_VALUES: find_related_codes}}}
