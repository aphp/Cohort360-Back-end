...
from cohort.services.emails import (
    ...
    send_email_notif_count_request_refreshed,
)
...
@shared_task
def create_cohort(cohort_id: str, json_query: str, auth_headers: dict, cohort_creator_cls: str, sampling_ratio: Optional[float] = None) -> None:
    ...
    cohort_creator.launch_cohort_creation(
        cohort_id=cohort_id, json_query=json_query, auth_headers=auth_headers, owner_username=cr.owner_id, sampling_ratio=sampling_ratio
    )
...
def count_cohort(
    dm_id: str, json_query: str, auth_headers: dict, cohort_counter_cls: str, global_estimate=False, stage_details: Optional[str] = None
) -> None:
    ...
    cohort_counter.launch_dated_measure_count(
        dm_id=dm_id,
...
def cancel_previous_count_jobs(dm_id: str, cohort_counter_cls: str):
    ...
    running_dms = request.dated_measures.exclude(uuid=dm_id).filter(
        request_job_status__in=(JobStatus.started, JobStatus.pending), cohorts__isnull=True, global_cohorts__isnull=True
    )
    cohort_counter = load_operator(cohort_counter_cls)
...
def send_feasibility_study_notification(feasibility_count_task_succeeded: bool, *args, **kwargs):
    ...
    send_email_notification(notification=notif, request_name=fs.request_query_snapshot.request.name, owner=fs.owner)
...
def send_email_feasibility_report_ready(fs_id: str) -> None:
    ...
    send_email_notification(
        notification=send_email_notif_feasibility_report_ready, request_name=fs.request_query_snapshot.request.name, owner=fs.owner, fs_id=fs_id
    )
...
def send_email_feasibility_report_error(fs_id: str) -> None:
    fs = get_feasibility_study_by_id(fs_id=fs_id)
    send_email_notification(
        notification=send_email_notif_error_feasibility_report, request_name=fs.request_query_snapshot.request.name, owner=fs.owner
    )
...
def send_email_count_request_refreshed(snapshot_id: str) -> None:
    ...
    send_email_notification(
        notification=send_email_notif_count_request_refreshed,
