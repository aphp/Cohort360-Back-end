...
from cohort.serializers import (
    CohortResultSerializer,
    CohortRightsSerializer,
    CohortResultPatchSerializer,
...
class CohortFilter(filters.FilterSet):
    ...
    ordering = OrderingFilter(
        fields=("created_at", "modified_at", "name", "result_size", "favorite", ("request_query_snapshot__request__name", "request"))
    )
...
class CohortResultViewSet(NestedViewSetMixin, UserObjectsRestrictedViewSet):
    queryset = CohortResult.objects.select_related("dated_measure", "dated_measure_global", "request_query_snapshot__request").annotate(
        request_id=F("request_query_snapshot__request__uuid"),
        ...
        exportable=Case(
            When(result_size__isnull=False, result_size__lte=settings.COHORT_SIZE_LIMIT, then=Value(True)),
            default=Value(False),
    ...
    non_updatable_fields = ["owner", "owner_id", "request_query_snapshot", "request_query_snapshot_id", "dated_measure", "dated_measure_id"]
    ...
    def partial_update(self, request, *args, **kwargs):
        if any(field in self.non_updatable_fields for field in request.data):
            return Response(data=f"The payload contains non-updatable fields `{request.data}`", status=status.HTTP_400_BAD_REQUEST)
    ...
    def get_active_jobs(self, request, *args, **kwargs):
        return Response(data={"jobs_count": cohort_service.count_active_jobs()}, status=status.HTTP_200_OK)
    ...
    def get_rights_on_cohorts(self, request, *args, **kwargs):
        cohorts_rights = cohort_rights_service.get_user_rights_on_cohorts(group_ids=request.query_params.get("group_id"), user=request.user)
