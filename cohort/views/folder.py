...
class FolderFilter(filters.FilterSet):
    ...
    class Meta:
        model = Folder
        fields = ["name", "min_created_at", "max_created_at"]
...
@extend_schema_view(
    ...
    create=extend_schema(request=FolderCreateSerializer, responses={status.HTTP_201_CREATED: FolderSerializer}),
    partial_update=extend_schema(request=FolderPatchSerializer, responses={status.HTTP_200_OK: FolderSerializer}),
