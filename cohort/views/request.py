...
@extend_schema_view(
    ...
    create=extend_schema(request=RequestCreateSerializer, responses={status.HTTP_201_CREATED: RequestSerializer}),
    partial_update=extend_schema(request=RequestPatchSerializer, responses={status.HTTP_200_OK: RequestSerializer}),
...
class RequestViewSet(NestedViewSetMixin, UserObjectsRestrictedViewSet):
    query_snapshots_subquery = RQS.objects.filter(request_id=OuterRef("uuid")).order_by("-created_at").values("created_at")[:1]
    queryset = Request.objects.prefetch_related(Prefetch(lookup="query_snapshots", queryset=RQS.objects.prefetch_related("cohort_results"))).annotate(
        updated_at=Subquery(query_snapshots_subquery.values("created_at"))
    )
