...
class TestCohortResultService(TestCaseWithDBs):
    def setUp(self):
        ...
        self.source_cohort = CohortResult.objects.create(owner=self.user1, request_query_snapshot=self.rqs, dated_measure=self.dm)
        self.fhir_filter = FhirFilter.objects.create(owner=self.user1, fhir_resource="Resource", name="Filter_01", filter="param=value")
    ...
    def test_create_cohort_subset(self, mock_get_auth_headers, mock_celery_task):
        ...
        cohort_subset = self.cohort_result_service.create_cohort_subset(
