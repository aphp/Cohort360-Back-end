...
class CohortsGetTests(CohortsTests):
    def setUp(self):
        ...
        self.rqss: List[RequestQuerySnapshot] = sum((list(u.user_request_query_snapshots.all()) for u in self.users), [])
        ...
        for i in range(200):
            ...
            dm = DatedMeasure(
                ...
                fhir_datetime=(timezone.now() - timedelta(days=random.randint(1, 5))),
            )
            to_create.append(dm)
        ...
        self.dms: List[DatedMeasure] = DatedMeasure.objects.bulk_create(to_create)
        self.crs: List[CohortResult] = CohortResult.objects.bulk_create(crs_to_create)
        self.active_jobs_url = self.objects_url + "jobs/active/"

    def test_list(self):
        # As a user, I can list the CR I own
        case = ListCase(to_find=[cr for cr in self.crs if cr.owner == self.user1], user=self.user1, success=True, status=status.HTTP_200_OK)
    ...
    def test_list_with_filters(self):
        ...
        cases = [
            basic_case.clone(params=dict(status=JobStatus.pending.value), to_find=[cr for cr in crs if cr.request_job_status == JobStatus.pending]),
            basic_case.clone(params=dict(name=self.str_pattern), to_find=[cr for cr in crs if self.str_pattern.lower() in cr.name.lower()]),
            basic_case.clone(params=dict(min_result_size=example_measure), to_find=[cr for cr in crs if cr.dated_measure.measure >= example_measure]),
            basic_case.clone(params=dict(max_result_size=example_measure), to_find=[cr for cr in crs if cr.dated_measure.measure <= example_measure]),
            basic_case.clone(params=dict(favorite=True), to_find=[cr for cr in crs if cr.favorite]),
            basic_case.clone(params=dict(group_id=first_cr.group_id), to_find=[first_cr]),
            basic_case.clone(params=dict(request_id=req.pk), to_find=sum((list(rqs.cohort_results.all()) for rqs in req.query_snapshots.all()), [])),
        ]
...
class CohortsCreateTests(CohortsTests):
    def setUp(self):
        ...
        self.user1_req1_snap1_dm = DatedMeasure.objects.create(
            owner=self.user1, request_query_snapshot=self.user1_req1_snap1, measure=1, fhir_datetime=timezone.now()
        )
        ...
        parent_cohort = CohortResult.objects.create(
            name="Parent cohort for sampling",
        ...
        self.basic_data_for_sampled_cohort = dict(
            name="Sampled Cohort", description="Sampled Cohort", parent_cohort=parent_cohort.uuid, sampling_ratio=0.3
        )
    ...
    def check_create_case_with_mock(
        self,
        case: CohortCreateCase,
        mock_count_task: MagicMock,
    ...
    def test_create_with_global(self):
        self.check_create_case(self.basic_case.clone(data={**self.basic_data, "global_estimate": True}))
    ...
    def test_error_create_missing_field(self):
        case = self.basic_err_case.clone(data={**self.basic_data, "request_query_snapshot": None}, success=False, status=status.HTTP_400_BAD_REQUEST)
        self.check_create_case(case)

    def test_error_create_with_rqs_not_owned(self):
        self.check_create_case(
            self.basic_err_case.clone(
                data={**self.basic_data, "request_query_snapshot": self.user2_req1_snap1.pk},
                success=False,
                status=status.HTTP_400_BAD_REQUEST,
            )
        )
    ...
    def test_successfully_create_sampled_cohort(self):
        self.check_create_case(
            self.basic_case.clone(
                data=self.basic_data_for_sampled_cohort, retrieve_filter=CohortCaseRetrieveFilter(name=self.basic_data_for_sampled_cohort.get("name"))
            )
        )

    def test_error_create_sampled_cohort_with_ratio_0(self):
        data = {**self.basic_data_for_sampled_cohort, "name": "Sampled Cohort with ratio 0.0", "sampling_ratio": 0.0}
        self.check_create_case(self.basic_err_case.clone(data=data, retrieve_filter=CohortCaseRetrieveFilter(name=data.get("name"))))

    def test_error_create_sampled_cohort_with_ratio_1(self):
        data = {**self.basic_data_for_sampled_cohort, "name": "Sampled Cohort with ratio 1.0", "sampling_ratio": 1.0}
        self.check_create_case(self.basic_err_case.clone(data=data, retrieve_filter=CohortCaseRetrieveFilter(name=data.get("name"))))

    def test_error_create_sampled_cohort_with_ratio_gt_1(self):
        data = {**self.basic_data_for_sampled_cohort, "name": "Sampled Cohort with ratio gt 1", "sampling_ratio": 2.5}
        self.check_create_case(self.basic_err_case.clone(data=data, retrieve_filter=CohortCaseRetrieveFilter(name=data.get("name"))))

    def test_error_create_sampled_cohort_with_ratio_lt_0(self):
        data = {**self.basic_data_for_sampled_cohort, "name": "Sampled Cohort with ratio lt 0", "sampling_ratio": -1.5}
        self.check_create_case(self.basic_err_case.clone(data=data, retrieve_filter=CohortCaseRetrieveFilter(name=data.get("name"))))
...
class CohortsDeleteTests(CohortsTests):
    ...
    def test_delete_multiple(self):
        target_uuids = []
        for i in range(5):
            req = CohortResult.objects.create(
                **dict(
                    owner=self.user1,
...
class CohortsUpdateTests(CohortsTests):
    def setUp(self):
        ...
        self.basic_case = PatchCase(initial_data=self.basic_data, status=status.HTTP_200_OK, success=True, user=self.user1, data_to_update={})

        self.basic_err_case = self.basic_case.clone(status=status.HTTP_400_BAD_REQUEST, success=False)
    ...
    def test_error_update_cohort_as_not_owner(self, mock_ws_send_to_client):
        mock_ws_send_to_client.return_value = None
        case = self.basic_err_case.clone(data_to_update=dict(name="new name"), user=self.user2, status=status.HTTP_404_NOT_FOUND)
    ...
    def test_error_update_cohort_forbidden_fields(self, mock_ws_send_to_client):
        ...
        cases = [
            self.basic_err_case.clone(data_to_update={k: v})
            for k, v in dict(owner=self.user2.pk, request_query_snapshot=self.user1_req1_branch1_snap2.pk, dated_measure=dm.pk).items()
        ]
    ...
    def test_patch_cohort_with_status_finished(self, mock_patch_handler, mock_post_update, mock_ws_send_to_client):
        ...
        mock_post_update.return_value = None
        mock_ws_send_to_client.return_value = None
        resp_data = self.check_patch_case(case=self.basic_case.clone(data_to_update={"request_job_status": "finished"}), return_response_data=True)
