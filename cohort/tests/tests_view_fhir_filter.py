...
class TestFhirFilters(CohortAppTests):
    ...
    def test_edit_name(self):
        ...
        fhir_filter = FhirFilter.objects.create(
            fhir_resource="Patient", fhir_version="1.0.0", name="original_name", filter='{"some": "filter"}', owner=self.user1
        )
    ...
    def test_edit_name_error(self):
        old_name = "filter_001"
        new_name = "filter_002"
        base_data = dict(fhir_resource="Patient", fhir_version="1.0.0", filter='{"some": "filter"}', owner=self.user1)
    ...
    def test_edit_name_only_modifies_name(self):
        user = User.objects.first()
        kwargs = {"fhir_resource": "Patient", "fhir_version": "1.0.0", "name": "original_name", "filter": '{"some": "filter"}', "owner": user}
    ...
    def test_hundred_of_filters_no_api(self):
        ...
        for i in range(loops):
            FhirFilter.objects.create(fhir_resource=f"res {i}", name="name", owner=user, filter='{"some": "filter"}', fhir_version="1.0.0")
    ...
    def test_hundred_of_filters_api_call(self):
        ...
        url = reverse("cohort:fhir-filters-list")
        for i in range(loops):
            data = {"fhir_resource": f"res {i}", "fhir_version": "1.0.0", "name": "test_filter", "filter": '{"some": "filter"}'}
            request = self.factory.post(url, data=data, format="json")
            force_authenticate(request, user)
    ...
    def test_delete_multiple_filters(self):
        ...
        for i in range(25):
            FhirFilter.objects.create(
                **{"owner": user, "fhir_resource": f"res {i}", "fhir_version": "1.0.0", "name": f"test_filter {i}", "filter": '{"some": "filter"}'}
            )
    ...
    def test_name_minimal_length(self):
        user = User.objects.first()
        FhirFilter.objects.create(fhir_resource="Resource 1", name="x" * 2, owner=user, filter='{"some": "filter"}', fhir_version="1.0.0")
        assert FhirFilter.objects.count() == 1

    def test_name_max_length(self):
        user = User.objects.first()
        FhirFilter.objects.create(fhir_resource="Resource 1", name="x" * 50, owner=user, filter='{"some": "filter"}', fhir_version="1.0.0")
        assert FhirFilter.objects.count() == 1

    def test_name_max_length_plus_one(self):
        user = User.objects.first()
        with pytest.raises(DataError):
            FhirFilter.objects.create(fhir_resource="Resource 1", name="x" * 51, owner=user, filter='{"some": "filter"}', fhir_version="1.0.0")

    def test_name_min_length_minus_one(self):
        user = User.objects.first()
        with pytest.raises(ValidationError):
            f = FhirFilter.objects.create(fhir_resource="Resource 1", name="x" * 1, owner=user, filter='{"some": "filter"}', fhir_version="1.0.0")
    ...
    def test_null_resource(self):
        with pytest.raises(IntegrityError):
            FhirFilter.objects.create(
                fhir_resource=None, name="name of filter", owner=User.objects.first(), filter='{"some": "filter"}', fhir_version="1.0.0"
            )

    def test_null_name(self):
        with pytest.raises(IntegrityError):
            FhirFilter.objects.create(
                fhir_resource="Resource 1", name=None, owner=User.objects.first(), filter='{"some": "filter"}', fhir_version="1.0.0"
            )

    def test_null_user(self):
        with pytest.raises(IntegrityError):
            FhirFilter.objects.create(
                fhir_resource="Resource 1", name="name of filter", owner=None, filter='{"some": "filter"}', fhir_version="1.0.0"
            )

    def test_null_filter(self):
        with pytest.raises(AttributeError):
            FhirFilter.objects.create(
                fhir_resource="Resource 1", name="name of filter", owner=User.objects.first(), filter=None, fhir_version="1.0.0"
            )
    ...
    def test_strip_filter(self):
        ...
        f = FhirFilter.objects.create(
            fhir_resource="Resource 1", name="name of filter", owner=User.objects.first(), filter=user_input, fhir_version="1.0.0"
        )
    ...
    def test_null_version(self):
        with pytest.raises(IntegrityError):
            FhirFilter.objects.create(
                fhir_resource="Resource 1", name="name of filter", owner=User.objects.first(), filter='{"some": "filter"}', fhir_version=None
            )
