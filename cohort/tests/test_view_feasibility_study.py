...
class FeasibilityStudyViewTests(CohortAppTests):
    ...
    def setUp(self):
        ...
        self.request = Request.objects.create(owner=self.user1, name="Request 1", description="Request 1 from user 1", parent_folder=self.folder)
        self.rqs = RequestQuerySnapshot.objects.create(owner=self.user1, request=self.request, serialized_query="{}")
        self.basic_data = dict(request_query_snapshot_id=self.rqs.pk, owner=self.user1)
    ...
    def test_successfully_patch_feasibility_study(self, mock_patch_handler, mock_build_report, mock_send_email_report_ready, mock_get_template):
        extra = {group_id: count for (group_id, count) in [("1", "10"), ("2", "10"), ("3", "10"), ("4", "15"), ("5", "15"), ("6", "25")]}
        ...
        patch_data = {"request_job_status": "finished", "count": 25, "extra": extra}
        case = PatchCase(initial_data=self.basic_data, data_to_update=patch_data, user=self.user1, status=status.HTTP_200_OK, success=True)
    ...
    def test_error_patch_feasibility_study(self, mock_patch_handler, mock_send_email_report_error):
        ...
        case = PatchCase(
            ...
            data_to_update={
                "request_job_status": "INVALID_STATUS",
            },
            user=self.user1,
    ...
    def test_download_report(self):
        ...
        f_study = FeasibilityStudy.objects.create(**self.basic_data, report_file=str.encode(html_report))
        url = reverse(viewname="cohort:feasibility-studies-download-report", args=[f_study.uuid])
        base_case = FileDownloadCase(
            url=url, view_params=dict(uuid=f_study.uuid), expected_content_type=None, user=self.user1, success=True, status=status.HTTP_200_OK
        )
        ...
        f_study.save()
        error_case = base_case.clone(success=False, status=status.HTTP_404_NOT_FOUND)
...
class TestFeasibilityStudiesService(TestCaseWithDBs):
    ...
    def create_perimeters_tree():
        basic_data = [
            ...
            {"id": 5, "name": "UF-01", "type_source_value": "TYPE4", "cohort_id": "5", "local_id": "5", "parent_id": 4, "level": 5},
            {"id": 6, "name": "UC-01", "type_source_value": "TYPE5", "cohort_id": "6", "local_id": "6", "parent_id": 5, "level": 6},
        ]
        for d in basic_data:
