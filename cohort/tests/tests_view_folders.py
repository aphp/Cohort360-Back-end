...
from admin_cohort.tests.tests_tools import CaseRetrieveFilter, random_str, ListCase, RetrieveCase, CreateCase, DeleteCase, PatchCase
...
class FoldersTests(CohortAppTests):
    ...
    unsettable_fields = ["owner", "uuid", "created_at", "modified_at", "deleted"]
...
class FoldersGetTests(FoldersTests):
    def setUp(self):
        ...
        self.folders: List[Folder] = Folder.objects.bulk_create(
            [
                Folder(name=random_str(10, random.random() > 0.5 and self.name_pattern), owner=self.user1 if i % 2 else self.user2)
                for i in range(0, nb_base)
            ]
        )

        for f in self.folders[:nb_base]:
            self.folders += Folder.objects.bulk_create([Folder(name=random_str(10, random.random() > 0.5 and self.name_pattern), owner=f.owner)])

    def test_list(self):
        # As a user, I can list the folders I own
        case = ListCase(to_find=[f for f in self.folders if f.owner == self.user1], user=self.user1, success=True, status=status.HTTP_200_OK)
...
class FoldersDeleteTests(FoldersTests):
    ...
    def test_delete_as_owner(self):
        ...
        self.check_delete_case(self.basic_case.clone(data_to_delete=dict(owner=self.user1, name="test")))
...
class FoldersUpdateTests(FoldersTests):
    ...
    def test_update_as_owner(self):
        ...
        initial_data = {"owner": self.user1, "name": "test"}
        data_to_update = {
            "name": "new_name",
            # read_only
            "created_at": timezone.now() + timedelta(hours=1),
