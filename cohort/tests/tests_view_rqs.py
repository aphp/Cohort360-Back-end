...
from admin_cohort.tests.tests_tools import CaseRetrieveFilter, CreateCase, random_str, ListCase, RetrieveCase, DeleteCase, PatchCase
...
class RqsTests(RequestsTests):
    unupdatable_fields = ["owner", "request", "uuid", "previous_snapshot", "shared_by", "created_at", "modified_at", "deleted"]
    ...
    update_view = RequestQuerySnapshotViewSet.as_view({"patch": "partial_update"})
    ...
    def setUp(self):
        ...
        self.user1_req1 = Request.objects.create(
            owner=self.user1, name="Request 1", description="Request 1 from user 1", parent_folder=self.user1_folder1
        )
        self.user1_req2 = Request.objects.create(
            owner=self.user1, name="Request 2", description="Request 2 from user 1", parent_folder=self.user1_folder2
        )
        self.user2_req1 = Request.objects.create(
            owner=self.user2, name="Request 1", description="Request 1 from user 2", parent_folder=self.user2_folder1
        )
        self.user2_req2 = Request.objects.create(
            owner=self.user2, name="Request 2", description="Request 2 from user 2", parent_folder=self.user2_folder2
        )
        self.requests = [self.user1_req1, self.user1_req2, self.user2_req1, self.user2_req2]
...
class RqsGetTests(RqsTests):
    def setUp(self):
        ...
        self.folders: List[Folder] = sum((list(u.folders.all()) for u in self.users), [])
        base_rqss: List[RequestQuerySnapshot] = []

        def random_json():
            return f'{"{"}"{random_str(5)}": "' + random_str(20, random.random() > 0.5 and self.str_pattern) + '"}'
        ...
        for r in self.requests:
            base_rqss.append(
                RequestQuerySnapshot(
                    ...
                    shared_by=(next(u for u in self.users if u != r.owner) if random.random() > 0.5 else None),
                )
            )
        ...
        for i in range(300):
            ...
            new_rqss.append(
                RequestQuerySnapshot(
                    ...
                    serialized_query=random_json(),
                    shared_by=(next(u for u in self.users if u != prev.owner) if random.random() > 0.5 else None),
                )
            )
    ...
    def test_list(self):
        ...
        case = ListCase(to_find=[r for r in self.rqss if r.owner == self.user1], user=self.user1, success=True, status=status.HTTP_200_OK)
    ...
    def test_list_with_filters(self):
        ...
        cases = [
            basic_case.clone(
                ...
                to_find=[rqs for rqs in user1_rqss if rqs.shared_by == self.user2],
            ),
            basic_case.clone(
            ...
            basic_case.clone(
                ...
                to_find=sum((list(r.query_snapshots.all()) for r in folder.requests.all()), []),
            ),
        ]
    ...
    def test_rest_get_list_from_request(self):
        ...
        self.check_get_paged_list_case(
            ListCase(status=status.HTTP_200_OK, success=True, user=self.user1, to_find=list(req.query_snapshots.all())),
    ...
    def test_rest_get_list_from_previous_rqs(self):
        ...
        # bound to
        prev_rqs = self.user1.folders.first().requests.first().query_snapshots.first()

        self.check_get_paged_list_case(
            ListCase(status=status.HTTP_200_OK, success=True, user=self.user1, to_find=list(prev_rqs.next_snapshots.all())),
            other_view=NestedRqsViewSet.as_view({"get": "list"}),
...
class RqsCreateTests(RqsTests):
    def check_create_case_with_mock(self, case: RqsCreateCase, other_view: any, view_kwargs: dict):
        ...
        super(RqsCreateTests, self).check_create_case(case, other_view, **(view_kwargs or {}))

    def check_create_case(self, case: RqsCreateCase, other_view: any = None, **view_kwargs):
    ...
    def setUp(self):
        ...
        self.basic_case = RqsCreateCase(
            ...
            retrieve_filter=RqsCaseRetrieveFilter(serialized_query=self.test_query),
        )
    ...
    def test_create_from_request(self):
        ...
        self.check_create_case(
            self.basic_case.clone(data={**self.basic_data, "request": None}),
    ...
    def test_create_on_previous(self):
        ...
        self.check_create_case(
            self.basic_case.clone(
                data={"serialized_query": self.test_query, "previous_snapshot": self.user1_req1_snap1.pk},
            )
        )
    ...
    def test_create_from_previous(self):
        ...
        # using nestedViewSet
        self.check_create_case(
            case=self.basic_case.clone(data={"serialized_query": self.test_query}),
    ...
    def test_error_create_unmatching_request_previous(self):
        ...
        self.check_create_case(
            self.basic_err_case.clone(
                data={**self.basic_data, "previous_snapshot": self.user1_req1_snap1.pk},
            )
        )
...
class RqsShareTests(RqsTests):
    ...
    def check_share_case(
        ...
        request = self.factory.post(f"{self.objects_url}/{obj.pk}/share", data=data, format="json")
        if case.user:
        ...
        if case.success:
            ...
            if case.recipients is not None:
                self.assertIsNone(RequestQuerySnapshot.objects.filter(owner__in=case.recipients, shared_by=case.user).first(), msg=msg)
    ...
    def test_mail_sending(self):
        ...
        with patch.object(
            EmailMessage, "send", self.check_mail(from_email=environ.get("DEFAULT_FROM_EMAIL"), to_emails=[self.user2.email], body=email_content)
        ):
            self.check_share_case(self.basic_case.clone(recipients=[self.user2]))
    ...
    def test_error_share_unknown_users(self):
        ...
        self.check_share_case(
            self.basic_case.clone(
                status=status.HTTP_400_BAD_REQUEST,
                success=False,
                recipients=[self.user2, User(pk="wrong", email="n@aphp.fr", firstname="not", lastname="existing")],
            )
        )

    def test_error_missing_recipients(self):
        # As a user, I cannot share a RQS without recipients
        self.check_share_case(self.basic_case.clone(status=status.HTTP_400_BAD_REQUEST, success=False, recipients=None))
