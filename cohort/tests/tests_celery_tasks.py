...
from cohort.services.emails import (
    send_email_notif_feasibility_report_requested,
...
class TasksTests(TestCaseWithDBs):
    def setUp(self):
        ...
        self.request = Request.objects.create(
            owner=self.user1, name="Request 01 with running DMs", description="Request with DMs started, pending", parent_folder=self.folder
        )
        self.req_snapshot = RequestQuerySnapshot.objects.create(owner=self.user1, request=self.request, serialized_query="{}")
        self.dm1 = DatedMeasure.objects.create(owner=self.user1, request_query_snapshot=self.req_snapshot)
        self.global_dm = DatedMeasure.objects.create(owner=self.user1, request_query_snapshot=self.req_snapshot, mode=GLOBAL_DM_MODE)
        self.cohort = CohortResult.objects.create(
            name="My small cohort",
        ...
        self.req_snapshot2 = RequestQuerySnapshot.objects.create(owner=self.user1, request=self.request, serialized_query="{}")
        self.new_dm = DatedMeasure.objects.create(request_query_snapshot=self.req_snapshot2, request_job_status=JobStatus.new, owner=self.user1)
        self.started_dm = DatedMeasure.objects.create(
            request_query_snapshot=self.req_snapshot2, request_job_status=JobStatus.started, owner=self.user1
        )
        self.pending_dm = DatedMeasure.objects.create(
            request_query_snapshot=self.req_snapshot2, request_job_status=JobStatus.pending, owner=self.user1
        )

        self.req_snapshot3 = RequestQuerySnapshot.objects.create(owner=self.user1, request=self.request, serialized_query="{}")
        self.dm2 = DatedMeasure.objects.create(request_query_snapshot=self.req_snapshot3, request_job_status=JobStatus.new, owner=self.user1)
        self.started_dm2 = DatedMeasure.objects.create(
            request_query_snapshot=self.req_snapshot3, request_job_status=JobStatus.started, owner=self.user1
        )
        self.user1_feasibility_study = FeasibilityStudy.objects.create(owner=self.user1, request_query_snapshot=self.req_snapshot)
    ...
    def test_count_cohort_task(self):
        with self.assertRaises(NotImplementedError):
            count_cohort(dm_id=self.dm1.uuid, json_query=self.json_query, cohort_counter_cls=self.cohort_counter_cls, auth_headers=self.auth_headers)
    ...
    def test_create_cohort_task(self):
        with self.assertRaises(NotImplementedError):
            create_cohort(
                cohort_id=self.cohort.uuid, json_query=self.json_query, cohort_creator_cls=self.cohort_creator_cls, auth_headers=self.auth_headers
            )

    def test_feasibility_study_count_task(self):
        success = feasibility_study_count(
    ...
    def test_feasibility_email_notification_tasks(self):
        ...
        cases = [
            ...
            {"task": send_feasibility_study_notification, "args": (False, fs_id), "notification_to_send": send_email_notif_error_feasibility_report},
            {"task": send_email_feasibility_report_ready, "args": (fs_id,), "notification_to_send": send_email_notif_feasibility_report_ready},
            {"task": send_email_feasibility_report_error, "args": (fs_id,), "notification_to_send": send_email_notif_error_feasibility_report},
        ]
        for case in cases:
    ...
    def test_cancel_previous_count_jobs_task(self, mock_celery_revoke):
        ...
        cancel_previous_count_jobs(dm_id=self.new_dm.uuid, cohort_counter_cls=self.cohort_counter_cls)
        cancelled_dms = DatedMeasure.objects.exclude(uuid=self.new_dm.uuid).filter(request_query_snapshot=self.req_snapshot2)
