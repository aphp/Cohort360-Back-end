...
class DatedMeasuresTests(RqsTests):
    unupdatable_fields = ["owner", "request_query_snapshot", "uuid", "mode", "count_task_id", "fhir_datetime", "created_at", "modified_at", "deleted"]
    unsettable_default_fields = {}
    unsettable_fields = ["owner", "uuid", "count_task_id", "created_at", "modified_at", "deleted"]
...
class DatedMeasuresGetTests(DatedMeasuresTests):
    ...
    def test_list(self):
        ...
        case = ListCase(to_find=[dm for dm in self.dms if dm.owner == self.user1], user=self.user1, success=True, status=status.HTTP_200_OK)
...
class DatedMeasuresCreateTests(DatedMeasuresTests):
    def setUp(self):
        ...
        self.req_with_running_dms = Request.objects.create(
            owner=self.user1, name="Request with running DMs", description="Request with DMs started, pending", parent_folder=self.user1_folder1
        )
        ...
        self.started_dm = DatedMeasure.objects.create(
            request_query_snapshot=self.user1_req_running_dms_snap1, request_job_status=JobStatus.started, owner=self.user1
        )
        self.pending_dm = DatedMeasure.objects.create(
            request_query_snapshot=self.user1_req_running_dms_snap1, request_job_status=JobStatus.pending, owner=self.user1
        )
    ...
    def check_create_case_with_mock(
        self, case: DMCreateCase, mock_count_task: MagicMock, mock_cancel_task: MagicMock, mock_header: MagicMock, other_view: any, view_kwargs: dict
    ):
    ...
    def test_error_create_missing_field(self):
        ...
        case = self.basic_err_case.clone(
            data={**self.basic_data, "request_query_snapshot_id": None}, mock_header_called=False, mock_cancel_task_called=False
        )
    ...
    def test_error_create_with_other_owner(self):
        ...
        self.check_create_case(
            self.basic_err_case.clone(data={**self.basic_data, "owner": self.user2.pk}, mock_header_called=False, mock_cancel_task_called=False)
        )

    def test_error_create_on_rqs_not_provided(self):
        # cannot create a DM without an RQS
        case = self.basic_err_case.clone(data={"request_query_snapshot_id": None}, mock_header_called=False, mock_cancel_task_called=False)
    ...
    def test_error_create_on_rqs_not_owned(self):
        ...
        case = self.basic_err_case.clone(
            data={"request_query_snapshot_id": self.user2_req1_snap1.pk}, mock_header_called=False, mock_cancel_task_called=False
        )
        self.check_create_case(case=case)

    def test_create_with_request_having_running_dms(self):
        # before create new DM, cancel any previously running ones
        case = self.basic_case.clone(
            data={"request_query_snapshot_id": self.user1_req_running_dms_snap1.pk},
...
class DMUpdateTests(DatedMeasuresTests):
    def setUp(self):
        ...
        self.basic_data = dict(request_query_snapshot_id=self.user1_req1_snap1.pk, mode=DATED_MEASURE_MODE_CHOICES[0][0], owner=self.user1)
    ...
    def check_dm_patch_case(self, mock_patch_handler, mock_ws_send_to_client, case: dict):
        ...
        resp_data = self.check_patch_case(case=case["patch_case"], return_response_data=True)
    ...
    def test_patch_dm_with_status_finished(self):
        normal_dm_case = {
            "patch_case": self.basic_case.clone(data_to_update={"request_job_status": "finished"}),
    ...
    def test_patch_global_dm_with_status_finished(self):
        global_dm_case = {
            "patch_case": self.basic_case.clone(
                initial_data={**self.basic_data, "mode": GLOBAL_DM_MODE}, data_to_update={"request_job_status": "finished"}
            ),
            "mock_patch_handler_called": True,
