...
POLICY_FIELDS_REQUIRED_IF_NOT_DEFAULT = ['ranger_hive_policy_db', 'ranger_hive_policy_db_tables',
...
def get_ranger_hive_policy(user: dict) -> RangerHivePolicy:
    if user['ranger_hive_policy_type'] == RHP_TYPE_DEFAULT_USER:
        if RHP_TYPE_DEFAULT_USER in ranger_hive_policies:
            ...
            p, _ = RangerHivePolicy.objects.get_or_create(policy_type=RHP_TYPE_DEFAULT_USER)
        ...
        if k not in ranger_hive_policies:
            p, _ = RangerHivePolicy.objects.get_or_create(policy_type=user['ranger_hive_policy_type'],
                                                          db=user['ranger_hive_policy_db'],
                                                          db_tables=user['ranger_hive_policy_db_tables'],
                                                          db_imagerie=user.get('ranger_hive_policy_db_imagerie'),
...
def yaml_to_db(yaml_content: str):
    ...
    for a in to_update:
        try:
            ...
            brat_port = isinstance(user.get('brat_port'), int) and user.get('brat_port') or None
            tensorboard_port = isinstance(user.get('tensorboard_port'), int) and user.get('tensorboard_port') or None
            airflow_port = isinstance(user.get('airflow_port'), int) and user.get('airflow_port') or None

            a.__dict__.update({"name": user['name'],
