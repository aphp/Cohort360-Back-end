...
class PerimeterViewTests(AccessesAppTestsBase):
    ...
    def setUp(self):
        super().setUp()
        self.base_case = ListCase(params={}, to_find=[], user=None, success=True, status=status.HTTP_200_OK)

    def test_get_manageable_perimeters(self):
        base_case = ListCase(params={}, to_find=[], user=None, status=status.HTTP_200_OK, success=True)
        ...
        def get_manageable_perimeters_1():
            ...
            return base_case.clone(user=self.user_z, to_find=top_manageable_perimeters_ids)
        ...
        def get_manageable_perimeters_2():
            ...
            top_manageable_perimeters_ids = [self.p3.id, self.p4.id, self.p5.id, self.p1.id, self.p8.id, self.p13.id, self.p14.id]
            return base_case.clone(user=self.user_t, to_find=top_manageable_perimeters_ids)

        case_1 = get_manageable_perimeters_1()
        case_2 = get_manageable_perimeters_2()

        for case in (case_1, case_2):
            response_data = self.check_list_case(case=case, other_view=PerimeterViewTests.get_manageable_perimeters_view, yield_response_data=True)
            perimeter_ids_in_response = [e.get("id") for e in response_data]
            self.assertEqual(sorted(perimeter_ids_in_response), sorted(case.to_find))
    ...
    def test_get_data_reading_rights_on_perimeters(self):
        ...
        """
        base_case = ListCase(params={}, to_find=[], user=self.user_y, status=status.HTTP_200_OK, success=True)
        ...
        def get_data_reading_rights_on_perimeters_1():
            data_read_rights_p0 = PerimeterReadRight(
                perimeter=self.p0,
            ...
            data_read_rights_p1 = PerimeterReadRight(
                perimeter=self.p1,
            ...
            data_read_rights_p2 = PerimeterReadRight(
                perimeter=self.p2,
                right_read_patient_nominative=True,
                right_read_patient_pseudonymized=True,
                right_search_patients_by_ipp=True,
            ...
            data_read_rights_p4 = PerimeterReadRight(
                perimeter=self.p4,
                right_read_patient_nominative=True,
                right_read_patient_pseudonymized=True,
                right_search_patients_by_ipp=True,
            ...
            expected_data_read_rights = [data_read_rights_p0, data_read_rights_p1, data_read_rights_p2, data_read_rights_p4]
            return base_case.clone(params={}, to_find=expected_data_read_rights)
        ...
        def get_data_reading_rights_on_perimeters_2():
            ...
            data_read_rights_p5 = PerimeterReadRight(
                perimeter=self.p5,
                right_read_patient_nominative=False,
                right_read_patient_pseudonymized=True,
                right_search_patients_by_ipp=True,
            ...
            data_read_rights_p9 = PerimeterReadRight(
                perimeter=self.p9,
                right_read_patient_nominative=True,
                right_read_patient_pseudonymized=True,
                right_search_patients_by_ipp=True,
            ...
            expected_data_read_rights = [data_read_rights_p1, data_read_rights_p5, data_read_rights_p9]

            target_local_ids = [self.p1.local_id, self.p5.local_id, self.p9.local_id]

            return base_case.clone(params={"local_id": ",".join(target_local_ids)}, to_find=expected_data_read_rights)

        case_1 = get_data_reading_rights_on_perimeters_1()
        case_2 = get_data_reading_rights_on_perimeters_2()

        for case in (case_1, case_2):
            response_results = self.check_get_paged_list_case(
                case=case,
    ...
    def check_list_case_with_mock(self, case: ListCase, mock_get_target_perimeters: MagicMock, check_mock_was_called=True):
        ...
        response_data = self.check_list_case(case=case, other_view=PerimeterViewTests.check_read_patient_data_rights_view, yield_response_data=True)
    ...
    def test_read_patient_data_rights_case_3(self):
        ...
        roles = [self.role_data_reader_pseudo, self.role_data_reader_nomi, self.role_data_reader_nomi]
        self.make_accesses_for_user(self.profile_z, perimeters, roles)
        cohort_ids = ",".join([self.p5.cohort_id, self.p8.cohort_id])
        case_1 = self.base_case.clone(
            user=self.user_z,
        ...
        case_2 = case_1.clone(
            params={"cohort_ids": cohort_ids, "mode": "min"},
            to_find={
                "allow_read_patient_data_nomi": False,
                "allow_lookup_opposed_patients": False,
                "allow_read_patient_without_perimeter_limit": False,
                "allow_read_practitioner": False,
            },
    ...
    def test_read_patient_data_rights_case_4(self):
        ...
        roles = [self.role_data_reader_pseudo, self.role_data_reader_nomi_pseudo, self.role_data_reader_nomi]
        self.make_accesses_for_user(self.profile_z, perimeters, roles)
    ...
    def test_read_patient_data_rights_case_5(self):
        ...
        roles = [self.role_data_reader_pseudo, self.role_data_reader_pseudo]
        self.make_accesses_for_user(self.profile_z, perimeters, roles)
    ...
    def test_read_patient_data_rights_missing_access_on_some_target_perimeters(self):
        ...
        cohort_ids = ",".join([self.p1.cohort_id, self.p5.cohort_id])
        case_with_mode_min = self.base_case.clone(
            user=self.user_t, params={"cohort_ids": cohort_ids, "mode": "min"}, success=False, status=status.HTTP_404_NOT_FOUND
        )
        case_with_mode_max = self.base_case.clone(
            user=self.user_t,
            params={"cohort_ids": cohort_ids, "mode": "max"},
            to_find={
                "allow_read_patient_data_nomi": False,
    ...
    def test_read_patient_data_rights_with_user_having_no_data_rights(self):
        ...
        roles = [self.role_data_accesses_manager_same_level, self.role_data_accesses_manager_inf_levels]
        self.make_accesses_for_user(self.profile_y, perimeters, roles)

        cohort_ids = ",".join([self.aphp.cohort_id, self.p2.cohort_id])
        case = self.base_case.clone(
            title="user without rights",
            user=self.user_y,
            params={"cohort_ids": cohort_ids, "mode": "min"},
    ...
    def test_read_patient_data_rights_without_read_mode(self):
        ...
        case = self.base_case.clone(title="missing mode param", user=self.user_w, params={}, status=status.HTTP_400_BAD_REQUEST, success=False)
        self.check_list_case_with_mock(case, check_mock_was_called=False)
    ...
    def test_read_patient_data_rights_with_wrong_read_mode(self):
        ...
        case = self.base_case.clone(
            title="wrong value for mode param",
            user=self.user_u,
