...
class RoleViewTests(AccessesAppTestsBase):
    ...
    def setUp(self):
        ...
        Access.objects.create(
            profile=profile_full_admin_on_aphp,
    ...
    def test_error_creating_role_as_non_full_admin(self):
        ...
        data = {**ALL_FALSY_RIGHTS, "name": role_name, "right_read_patient_nominative": True, "right_search_patients_by_ipp": True}
        case = CreateCase(
            data=data,
            retrieve_filter=RoleRetrieveFilter(name=role_name),
    ...
    def test_successfully_patch_role(self):
        ...
        patch_data = {"name": "USERS MANAGER & LOGS READER", "right_manage_users": True}
        case = PatchCase(
            initial_data=initial_data, data_to_update=patch_data, user=self.user_full_admin_on_aphp, status=status.HTTP_200_OK, success=True
        )
        self.check_patch_case(case)

    def test_error_patching_role_as_non_full_admin(self):
        case = PatchCase(initial_data={}, data_to_update={}, user=self.user_non_full_admin, status=status.HTTP_403_FORBIDDEN, success=False)
        self.check_patch_case(case)
    ...
    def test_patch_role_with_inconsistent_rights(self):
        ...
        patch_data = {"name": "DATA READER PSEUDO & EXPORT NOMI", "right_export_csv_xlsx_nominative": True}
        case = PatchCase(
            initial_data=initial_data, data_to_update=patch_data, user=self.user_full_admin_on_aphp, status=status.HTTP_400_BAD_REQUEST, success=False
        )
        self.check_patch_case(case)

    def test_get_assignable_roles_missing_perimeter_id(self):
        case = ListCase(to_find=["Does not matter"], user=self.user_full_admin_on_aphp, status=status.HTTP_400_BAD_REQUEST, success=False)
    ...
    def test_get_assignable_roles_on_perimeter_APHP_as_full_admin_on_APHP(self):
        ...
        """
        case = ListCase(
            params={"perimeter_id": self.aphp.id},
    ...
    def test_get_assignable_roles_on_any_child_perimeter_of_APHP_as_full_admin_on_APHP(self):
        ...
        cases = [
            ListCase(
                title="get assignable roles on perimeter P5",
            ...
            ListCase(
                title="get assignable roles on perimeter P7",
            ...
            ListCase(
                title="get assignable roles on perimeter P13",
    ...
    def test_get_assignable_roles_on_perimeter_P0_as_admin_accesses_manager_on_APHP(self):
        ...
        case = ListCase(
            params={"perimeter_id": self.p0.id},
            to_find=[
    ...
    def test_get_assignable_roles_on_perimeter_P0_as_admin_accesses_manager_on_P4(self):
        ...
        case = ListCase(
            params={"perimeter_id": self.p0.id}, to_find=[], user=user_admin_accesses_manager_on_p4, status=status.HTTP_200_OK, success=False
        )
        self.check_list_case(case, other_view=RoleViewTests.assignable_view)
