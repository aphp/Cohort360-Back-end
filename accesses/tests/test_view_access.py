...
class AccessViewTests(AccessesAppTestsBase):
    ...
    def setUp(self):
        ...
        self.basic_access_data = {"profile_id": profile3.id, "role_id": self.role_data_reader_nomi.id, "perimeter_id": self.aphp.id}
    ...
    def test_successfully_create_access(self):
        data = {**self.basic_access_data, "start_datetime": timezone.now(), "end_datetime": timezone.now() + timedelta(days=10)}
        case = CreateCase(
            data=data,
    ...
    def test_successfully_create_access_with_empty_end_datetime(self):
        data = {**self.basic_access_data, "start_datetime": timezone.now()}
        case = CreateCase(
            data=data,
            user=self.user_data_accesses_manager_on_aphp,
            status=status.HTTP_201_CREATED,
    ...
    def test_error_create_access_with_past_start_end_datetime(self):
        ...
        data = {**self.basic_access_data, "start_datetime": past_datetime}
        case_with_past_start_datetime = CreateCase(
            data=data,
            user=self.user_data_accesses_manager_on_aphp,
        ...
        case_with_past_end_datetime = case_with_past_start_datetime.clone(
            data={**data, "start_datetime": timezone.now(), "end_datetime": past_datetime}
        )
    ...
    def test_successfully_close_access(self):
        initial_data = {**self.basic_access_data, "start_datetime": timezone.now(), "end_datetime": timezone.now() + timedelta(weeks=1)}
        case = PatchCase(
            initial_data=initial_data, data_to_update={}, user=self.user_data_accesses_manager_on_aphp, status=status.HTTP_200_OK, success=True
        )
        self.check_close_case(case)

    def test_error_close_a_future_access(self):
        initial_data = {
            **self.basic_access_data,
    ...
    def test_error_close_an_already_closed_access(self):
        initial_data = {**self.basic_access_data, "start_datetime": timezone.now(), "end_datetime": timezone.now() + timedelta(seconds=1)}
        time.sleep(5)
        case = PatchCase(
            initial_data=initial_data,
            data_to_update={},
    ...
    def test_successfully_patch_a_future_access(self):
        ...
        shift_dates_one_week = {"start_datetime": timezone.now() + timedelta(weeks=3), "end_datetime": timezone.now() + timedelta(weeks=4)}
        self.check_patch_case(
            PatchCase(
                initial_data=initial_data,
    ...
    def test_error_patch_a_future_access_with_wrong_dates(self):
        ...
        swap_dates = {"start_datetime": initial_data["end_datetime"], "end_datetime": initial_data["start_datetime"]}
        self.check_patch_case(
            PatchCase(
                initial_data=initial_data,
    ...
    def test_delete_accesses(self):
        ...
        case_started_access = DeleteCase(
            data_to_delete=dict(
                perimeter=self.p5,
                role=self.role_data_reader_nomi_pseudo,
    ...
    def test_list_accesses_of_user_x_as_user_y(self):
        ...
        for r, p in zip(user_x_roles, user_x_perimeters):
            Access.objects.create(
                profile=profile_x, role=r, perimeter=p, start_datetime=timezone.now(), end_datetime=timezone.now() + timedelta(weeks=1)
            )

        base_case = ListCase(params={"profile_id": profile_x.id}, to_find=[], user=self.user_y, status=status.HTTP_200_OK, success=True)
        ...
        def test_as_user_y_is_full_admin_on_aphp():
            ...
            resp_results = self.check_get_paged_list_case(base_case.clone(to_find=to_find), yield_response_results=True)
    ...
    def test_list_accesses_on_perimeter_P_for_user_y(self):
        ...
        def test_list_accesses_on_p2_as_user_y_is_full_admin_on_aphp():
            ...
            to_find_2 = Access.objects.filter(Q(perimeter_id=target_perimeter_id) | Q(perimeter_id=self.aphp.id))
    ...
    def test_successfully_get_my_accesses(self):
        ...
        case = ListCase(params={}, to_find=self.profile_y.accesses.all(), user=self.user_y, status=status.HTTP_200_OK, success=True)
    ...
    def test_successfully_get_my_valid_accesses_only(self):
        ...
        invalid_access_for_user_y = Access.objects.create(
            profile=self.profile_y,
            role=self.role_data_reader_nomi_csv_xlsx_exporter_nomi,
            perimeter=self.p9,
    ...
    def test_get_expiring_accesses(self):
        self.create_new_access_for_user(profile=self.profile_y, role=self.role_data_reader_nomi_pseudo, perimeter=self.p8)
        expiring_access_for_user_y = Access.objects.create(
            profile=self.profile_y,
            role=self.role_data_reader_nomi_csv_xlsx_exporter_nomi,
            perimeter=self.p9,
            start_datetime=timezone.now(),
            end_datetime=timezone.now() + timedelta(days=settings.ACCESS_EXPIRY_FIRST_ALERT_IN_DAYS - 1),
        )
        case_1 = ListCase(
            params={"expiring": "true"}, to_find=[expiring_access_for_user_y], user=self.user_y, status=status.HTTP_200_OK, success=True
        )
        case_2 = case_1.clone(params={"expiring": "false"}, to_find=self.profile_y.accesses.all())
        for case in (case_1, case_2):
            self.check_list_case(case, other_view=AccessViewTests.get_my_accesses_view)
    ...
    def test_get_my_data_reading_rights(self):
        ...
        for perimeters_ids, to_find in zip(target_perimeters_ids, to_find_list):
            case = base_case.clone(params={"perimeters_ids": ",".join(map(str, perimeters_ids))}, to_find=to_find)
            resp_data = self.check_list_case(case=case, other_view=AccessViewTests.get_my_data_reading_rights_view, yield_response_data=True)
