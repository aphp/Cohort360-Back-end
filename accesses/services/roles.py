...
ROLES_HELP_TEXT = {
    ...
    "right_read_patient_pseudonymized": "Lire les données patient sous forme pseudonymisée sur son périmètre et ses sous-périmètres",
    "right_search_patients_by_ipp": "Utiliser une liste d'IPP comme critère d'une requête Cohort.",
    "right_search_opposed_patients": "Détermine le droit de chercher les patients opposés à l'utilisation de leurs données pour la recherche",
...
class RolesService:
    ...
    def get_help_text_for_right_manage_admin_accesses(self, role):
        return self.build_help_text(
            text_root="Gérer les accès des administrateurs",
    ...
    def get_help_text_for_right_manage_data_accesses(self, role):
        return self.build_help_text(
            text_root="Gérer les accès aux données patients",
    ...
    def get_help_text(self, role):
        hierarchy_agnostic_rights = [
            right for right in all_rights if not (right.endswith("same_level") or right.endswith("inferior_levels") or right.endswith("above_levels"))
        ]
        help_txt = [ROLES_HELP_TEXT.get(r) for r in hierarchy_agnostic_rights if getattr(role, r, False)]

        hierarchy_dependent_texts = [
            self.get_help_text_for_right_manage_admin_accesses(role),
    ...
    def check_role_has_inconsistent_rights(data: dict) -> None:
        ...
        def is_full_admin_with_falsy_rights(d: dict) -> bool:
            return d.get("right_full_admin", False) and any(not d.get(right) for right in all_rights)

        def allow_read_data_pseudo_and_export_nomi(d: dict) -> bool:
            return (
                not d.get("right_read_patient_nominative", False)
                and d.get("right_read_patient_pseudonymized", False)
                and (d.get("right_export_csv_xlsx_nominative", False) or d.get("right_export_jupyter_nominative", False))
            )

        def allow_search_by_ipp_but_not_read_nomi(d: dict) -> bool:
            return d.get("right_search_patients_by_ipp", False) and not d.get("right_read_patient_nominative", False)

        def allow_manage_accesses(d: dict) -> bool:
            return any(
                (
                    d.get("right_manage_data_accesses_same_level", False),
    ...
    def get_assignable_roles_ids(user: User, perimeter_id: str, all_roles: QuerySet) -> List[int]:
        ...
        assignable_roles_ids = [
            role.id for role in all_roles if accesses_service.can_user_manage_access(user=user, target_access={"role": role, "perimeter": perimeter})
        ]
