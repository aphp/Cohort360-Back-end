    ...
    def list_rights():
        ...
        for right in Right.objects.all():
            if right.category not in right_categories:
                right_categories[right.category] = {
                    "name": right.category,
                    "is_global": right.is_global,
                    "rights": [{"name": right.name, "label": right.label, "depends_on": right.depends_on and right.depends_on.name or None}],
                }
            else:
                right_categories[right.category]["rights"].append(
                    {"name": right.name, "label": right.label, "depends_on": right.depends_on and right.depends_on.name or None}
                )
