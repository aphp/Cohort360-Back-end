...
from accesses.q_expressions import (
    q_allow_search_patients_by_ipp,
...
class PerimetersService:
    ...
    @staticmethod
    def get_top_perimeters_ids_inf_levels(
        inf_levels_perimeters_ids: List[int], all_perimeters_ids: List[int], top_same_level_perimeters_ids: Set[int]
    ) -> Set[int]:
        """
    ...
    def get_top_manageable_perimeters(self, user: User) -> QuerySet:
        ...
        if accesses_service.user_is_full_admin(user=user) or all(
            access.role.has_any_global_management_right() and not access.role.has_any_level_dependent_management_right() for access in user_accesses
        ):
            ...
            top_same_level_perimeters_ids = self.get_top_perimeters_ids_same_level(
                same_level_perimeters_ids=same_level_perimeters_ids, all_perimeters_ids=all_perimeters_ids
            )
            top_inf_levels_perimeters_ids = self.get_top_perimeters_ids_inf_levels(
    ...
    def get_target_perimeters(cohort_ids: str) -> QuerySet:
        ...
        virtual_cohorts = [i for v in virtual_cohorts_map.values() for i in v]
    ...
    @staticmethod
    def get_top_perimeters_with_read_pseudo_right(top_read_nomi_perimeters_ids: List[int], read_pseudo_perimeters_ids: List[int]) -> List[int]:
        ...
        for p in Perimeter.objects.filter(id__in=read_pseudo_perimeters_ids):
            if any(
                (parent_id in read_pseudo_perimeters_ids or parent_id in top_read_nomi_perimeters_ids or p.id in top_read_nomi_perimeters_ids)
                for parent_id in p.above_levels
            ):
                try:
    ...
    @staticmethod
    def get_perimeters_read_rights(
        ...
        return sorted(perimeter_read_rights, key=lambda x: (x.perimeter.full_path, x.perimeter.id))
    ...
    def get_data_read_rights_on_perimeters(self, user: User, is_request_filtered: bool, filtered_perimeters: QuerySet):
        ...
        read_nomi_perimeters_ids = user_accesses.filter(q_allow_read_patient_data_nominative).values_list("perimeter_id", flat=True)
        read_pseudo_perimeters_ids = user_accesses.filter(q_allow_read_patient_data_pseudo | q_allow_read_patient_data_nominative).values_list(
            "perimeter_id", flat=True
        )
        ...
        top_read_pseudo_perimeters_ids = self.get_top_perimeters_with_read_pseudo_right(
            top_read_nomi_perimeters_ids=top_read_nomi_perimeters_ids, read_pseudo_perimeters_ids=list(read_pseudo_perimeters_ids)
        )
        ...
        data_reading_rights = self.get_perimeters_read_rights(
...
def count_allowed_users():
    ...
    perimeters_with_counts = (
        Access.objects.select_related("profile", "perimeter")
        .filter(accesses_service.q_access_is_valid() & Q(profile__is_active=True))
        .values("perimeter_id")
...
def count_allowed_users_from_above_levels():
    ...
    valid_accesses = (
        Access.objects.filter(accesses_service.q_access_is_valid() & Q(profile__is_active=True) & q_impact_inferior_levels())
...
def process_leaf_perimeters():
    leaf_perimeters = Perimeter.objects.filter(Q(inferior_levels_ids="") | Q(inferior_levels_ids__isnull=True))
...
def count_allowed_users_in_inferior_levels():
    ...
    non_leaf_perimeters = (
        Perimeter.objects.filter(~Q(inferior_levels_ids="") & Q(inferior_levels_ids__isnull=False))
        .only("id", "inferior_levels_ids", "level")
        .order_by("-level")
    )

    valid_accesses = (
        Access.objects.filter(accesses_service.q_access_is_valid() & Q(profile__is_active=True)).values("perimeter_id", "profile__user_id").distinct()
    )
    users_per_perimeter = group_users_by_perimeter(valid_accesses)
    ...
    for perimeter in non_leaf_perimeters:
        ...
        for child_id in perimeter.inferior_levels:
            users_from_inferior_levels = users_from_inferior_levels_per_perimeter.get(child_id, users_per_perimeter.get(child_id, set()))
