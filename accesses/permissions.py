...
class AccessesPermission(permissions.BasePermission):
    ...
    def has_object_permission(self, request, view, obj):
        return request.method in ["GET", "PATCH", "DELETE"] and accesses_service.can_user_manage_access(user=request.user, target_access=obj)
...
class ProfilesPermission(permissions.BasePermission):
    ...
    def has_object_permission(self, request, view, obj):
        return request.method in ("GET", "PATCH") and self.has_permission(request, view)
