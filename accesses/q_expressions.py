...
q_allow_export_jupyter_pseudo = join_qs([Q(role__right_export_jupyter_nominative=True), Q(role__right_export_jupyter_pseudonymized=True)])


@lru_cache(maxsize=None)
def q_allow_manage_accesses_on_same_level() -> Q:
    return join_qs([Q(**{f"role__{right.name}": True}) for right in Right.objects.filter(allow_edit_accesses_on_same_level=True)])


@lru_cache(maxsize=None)
def q_allow_manage_accesses_on_inf_levels() -> Q:
    return join_qs([Q(**{f"role__{right.name}": True}) for right in Right.objects.filter(allow_edit_accesses_on_inf_levels=True)])


@lru_cache(maxsize=None)
def q_impact_inferior_levels() -> Q:
    return join_qs([Q(**{f"role__{right.name}": True}) for right in Right.objects.filter(impact_inferior_levels=True)])
