...
def re_compute_allowed_users_inferior_levels(perimeter):
    ...
    for child in perimeter.children.all():
        allowed_users_inferior_levels += (child.allowed_users or [])
        allowed_users_inferior_levels += re_compute_allowed_users_inferior_levels(perimeter=child)
    perimeter.allowed_users_inferior_levels = list(set(allowed_users_inferior_levels))
    perimeter.save()
...
def compute_allowed_users_from_inferior_levels(apps, schema_editor):
    ...
    for perimeter in perimeter_model.objects.using(db_alias).filter(level=1):
        re_compute_allowed_users_inferior_levels(perimeter)
