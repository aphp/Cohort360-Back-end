...
def compute_allowed_users(apps, schema_editor):
    ...
    _logger.info("perimeters_users count: ", len(perimeters_users))
...
def compute_allowed_users_from_inferior_levels(apps, schema_editor):
    ...
    almost_max_level = 6   # before last (max levels = 7)
    ...
    _logger.info("count_level_6: ", count_level_6)

    for level in range(almost_max_level-1, -1, -1):
        _logger.info("level: ", level)
        c = 0
        for perimeter in perimeter_model.objects.using(db_alias).filter(level=level):
            extra_users = set(uid for child in perimeter.children.all() for uid in (child.allowed_users_inferior_levels or []))
            perimeter.allowed_users_inferior_levels = list(extra_users)
            perimeter.save()
            c += 1
        _logger.info(f"count level {level}:", c)
