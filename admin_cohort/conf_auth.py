...
def get_token_from_headers(request) -> (str, str):
    ...
    if isinstance(header_authorization_method, str):
        header_authorization_method = header_authorization_method.encode(
...
def get_user_info(jwt_access_token: str) -> UserInfo:
    ...
    resp = requests.post(
        "{}/jwt/user_info/".format(JWT_SERVER_URL),
        data={"token": jwt_access_token}, headers=jwt_server_headers)
    if resp.status_code == 200:
...
def verify_jwt(access_token: str, auth_method: str = JWT_AUTH_MODE
    ...
    if auth_method.lower() == JWT_AUTH_MODE:
        ...
        if resp.status_code != status.HTTP_200_OK:
            raise ServerError(f"Error {resp.status_code} from "
                              f"authentication server ({OIDC_CERTS_URL}): "
                              f"{resp.text}")
        ...
        decoded = jwt.decode(access_token, verify=True, key=key,
                             algorithms=['RS256'], issuer=OIDC_SERVER_URL,
...
def refresh_jwt(refresh) -> JwtTokens:
    ...
    resp = requests.post(
        "{}/jwt/refresh/".format(JWT_SERVER_URL),
        data=dict(refresh=refresh), headers=jwt_server_headers)
    if resp.status_code == 200:
        return JwtTokens(**resp.json())
