...
def get_project_info():
    ...
    return (project_info.get("name"), project_info.get("version"), project_info.get("description"))
...
CORS_ALLOW_HEADERS = ["access-control-allow-origin", "content-type", "Authorization", "X-CSRFToken"]

ALLOWED_HOSTS = ["localhost", "127.0.0.1", "0.0.0.0", BACKEND_HOST]
...
ORBIS_ACCESSES_SYNC_ENABLED = env.bool("ORBIS_ACCESSES_SYNC_ENABLED", default=True)

if SCHEDULED_TASKS:
    CELERY_BEAT_SCHEDULE.update(
        **{
            task_name: {"task": task, "schedule": crontab(hour=hour, minute=minute)}
            for (task_name, task, hour, minute) in [task.strip().split(",") for task in SCHEDULED_TASKS.split(";")]
        }
    )
...
# CACHE
CACHES = {"default": {"BACKEND": "django_redis.cache.RedisCache", "LOCATION": CELERY_BROKER_URL}}
...
# WebSockets
WEBSOCKET_MANAGER = {"module": "admin_cohort.services.ws_event_manager", "manager_class": "WebsocketManager"}
