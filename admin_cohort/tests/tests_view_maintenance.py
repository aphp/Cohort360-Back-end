...
class MaintenanceTests(ViewSetTestsWithBasicPerims):
    ...
    def setUp(self):
        ...
        self.role_all_but_edit_maintenances = Role.objects.create(**dict([(r, True) for r in self.all_rights if r != "right_full_admin"]))
        Access.objects.create(
...
class MaintenanceGetTests(MaintenanceTests):
    def setUp(self):
        ...
        # can_read_mtnces
        self.user_with_no_right, self.prof_with_no_right = new_user_and_profile()
...
class MaintenanceGetListTests(MaintenanceGetTests):
    def setUp(self):
        ...
        self.list_mtnces: List[MaintenancePhase] = MaintenancePhase.objects.bulk_create(
            [
                MaintenancePhase(
                    subject=name,
                    start_datetime=(self.ref_now + timedelta(days=random.randint(-10, 10))),
                    end_datetime=(self.ref_now + timedelta(days=random.randint(-10, 10))),
                )
    ...
    def test_get_all_mtnces(self):
        ...
        case = ListCase(to_find=[*self.list_mtnces], success=True, status=status.HTTP_200_OK, user=self.user_with_no_right)
    ...
    def test_get_list_with_params(self):
        ...
        basic_case_dict = dict(success=True, status=status.HTTP_200_OK, user=self.user_with_no_right)
        cases = [
            ListCase(
                # - start_datetime
                **basic_case_dict,
                title=f"start_datetime={self.ref_now}",
                to_find=[mtnce for mtnce in self.list_mtnces if self.ref_now == mtnce.start_datetime],
            ...
            ListCase(
                ...
                **basic_case_dict,
                ...
                to_find=[mtnce for mtnce in self.list_mtnces if self.ref_now == mtnce.end_datetime],
            ...
            ListCase(
                ...
                **basic_case_dict,
                ...
                to_find=[mtnce for mtnce in self.list_mtnces if str(self.name_pattern) in str(mtnce.subject)],
...
class MaintenanceGetNextTests(MaintenanceGetTests):
    ...
    @staticmethod
    def prepare_maintenances(nb_days: List[Tuple[int, int]]) -> List[MaintenancePhase]:
        return MaintenancePhase.objects.bulk_create(
            [
                MaintenancePhase(start_datetime=timezone.now() + timedelta(days=m), end_datetime=timezone.now() + timedelta(days=n))
                for (m, n) in nb_days
            ]
        )
...
class MaintenanceCreateTests(MaintenanceTests):
    ...
    def test_error_create_wrong_mtnc(self):
        ...
        cases = [
            ...
            for (d, t) in [
                ({**self.creation_data, "start_datetime": (self.creation_data.get("end_datetime") + timedelta(seconds=1))}, "start > end"),
                # ({**self.creation_data,
...
class MaintenancePatchTests(MaintenanceTests):
    ...
    def test_patch_as_user_admin(self):
        ...
        cases = [
            self.basic_patch_case.clone(user=self.user_maintenance_manager, success=True, status=status.HTTP_200_OK, data_to_update=dict({k: v}))
    ...
    def test_error_wrong_data(self):
        ...
        cases = [
            ...
            for (d, t) in [
                (
                    {**self.base_data_to_update, "start_datetime": (self.base_data_to_update.get("end_datetime") + timedelta(seconds=1))},
                    "start > end",
                ),
                ({"start_datetime": (self.created_data.get("end_datetime") + timedelta(seconds=1))}, "start > end but patching start only"),
                ({"end_datetime": (self.created_data.get("start_datetime") - timedelta(seconds=1))}, "start > end but patching end only"),
