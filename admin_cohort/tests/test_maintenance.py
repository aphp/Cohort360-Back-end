...
class TestMaintenanceService(TestCase):
    ...
    def test_send_deleted_maintenance_notification_within_time_range(self, mock_send_notification, mock_phase_to_info):
        ...
        mock_phase_to_info.return_value = WSMaintenanceInfo(
            id=1,
            maintenance_start=str(maintenance_info.start_datetime),
            maintenance_end=str(maintenance_info.end_datetime),
            active=True,
            type="test",
            subject="test",
            message="test",
    ...
    def test_send_maintenance_notification_active(self, mock_send_to_client, mock_filter, mock_parse):
        now = timezone.now()
        maintenance_info = WSMaintenanceInfo(
            id=1,
            maintenance_start=str(now - timedelta(minutes=5)),
            maintenance_end=str(now + timedelta(minutes=5)),
            active=True,
            type="test",
            subject="test",
            message="test",
    ...
    def test_send_maintenance_notification_inactive_with_current_active(self, mock_send_to_client, mock_filter, mock_parse):
        ...
        maintenance_info = WSMaintenanceInfo(
            id=1,
            maintenance_start=str(now - timedelta(minutes=10)),
            maintenance_end=str(now - timedelta(minutes=5)),
            active=False,
            type="test",
            subject="test",
