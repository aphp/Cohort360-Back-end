...
def create_regular_user() -> User:
    return User.objects.create(firstname="Regular", lastname="USER", email="regular.user@aphp.fr", username="12345")
...
class JWTLoginTests(AuthBaseTests):
    def setUp(self):
        ...
        self.unregistered_user_credentials = {"username": "spy-user", "password": "top-secret-007"}
    ...
    def test_login_with_unregistered_user(self, mock_check_credentials: MagicMock):
        ...
        request = self.factory.post(path=self.login_url, data=self.unregistered_user_credentials, headers=self.headers)
        self._add_session_to_request(request)
    ...
    def test_login_with_wrong_credentials(self, mock_check_credentials: MagicMock):
        ...
        request = self.factory.post(
            path=self.login_url, data={"username": self.regular_user.username, "password": "wrong-psswd"}, headers=self.headers
        )
        self._add_session_to_request(request)
        response = LoginView.as_view({"post": "post"})(request)
    ...
    def test_login_unavailable_id_checker_server(self, mock_check_credentials: MagicMock):
        ...
        request = self.factory.post(path=self.login_url, data={"username": self.regular_user.username, "password": "psswd"}, headers=self.headers)
        self._add_session_to_request(request)
        response = LoginView.as_view({"post": "post"})(request)
        mock_check_credentials.assert_called()
        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)
    ...
    def test_login_success(self, mock_check_credentials: MagicMock):
        mock_check_credentials.return_value = True
        request = self.factory.post(
            path=self.login_url, data={"username": self.regular_user.username, "password": "any-will-do"}, headers=self.headers
        )
        self._add_session_to_request(request)
        response = LoginView.as_view({"post": "post"})(request)
        mock_check_credentials.assert_called()
...
if settings.ENABLE_OIDC_AUTH:
    ...
    class OIDCLoginTests(AuthBaseTests):
        ...
        def test_login_without_auth_code(self):
            response = self.client.post(path=self.login_url, content_type="application/json", data={"not_auth_code": "value"}, headers=self.headers)
            self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)
        ...
        def test_login_success(self, mock_get_tokens: MagicMock, mock_retrieve_username: MagicMock):
            ...
            response = self.client.post(
                ...
                data={"auth_code": "any-auth-code-will-do", "redirect_uri": "some-redirect-url"},
...
class RefreshTokenTests(AuthBaseTests):
    ...
    def test_refresh_token_method_not_allowed(self):
        request = self.factory.get(
            ...
            headers={"Authorization": "Bearer any-auth", settings.AUTHORIZATION_METHOD_HEADER: settings.OIDC_AUTH_MODE},
        )
    ...
    def test_refresh_token_with_invalid_auth_mode(self):
        with self.assertRaises(KeyError):
            request = self.factory.post(
                ...
                content_type="application/json",
                data={"refresh_token": "any-auth-code-will-do"},
                headers={"Authorization": "Bearer any-auth", settings.AUTHORIZATION_METHOD_HEADER: "INVALID_AUTH_MODE"},
            )
            self._add_session_to_request(request)
            force_authenticate(request, self.regular_user)
    ...
    def test_refresh_token_with_jwt_auth_mode(self, mock_refresh_token: MagicMock):
        ...
        request = self.factory.post(
            ...
            headers={"Authorization": "Bearer any-auth", settings.AUTHORIZATION_METHOD_HEADER: settings.JWT_AUTH_MODE},
        )
        self._add_session_to_request(request)
        force_authenticate(request, self.regular_user)
...
class LogoutTests(AuthBaseTests):
    ...
    def test_success_logout_from_jwt_session(self, mock_logout: MagicMock):
        request = self.factory.post(path=self.logout_url, headers={"AuthorizationMethod": settings.JWT_AUTH_MODE})
        self._add_session_to_request(request)
        force_authenticate(request, self.regular_user)
    ...
    def test_success_logout_from_oidc_session(self, mock_logout: MagicMock):
        request = self.factory.post(
            path=self.logout_url, data={"refresh_token": "any-token-will-do"}, headers={settings.AUTHORIZATION_METHOD_HEADER: settings.OIDC_AUTH_MODE}
        )
        self._add_session_to_request(request)
        force_authenticate(request, self.regular_user)
        mock_logout.return_value = None
