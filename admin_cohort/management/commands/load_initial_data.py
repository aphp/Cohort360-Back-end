...
class Command(BaseCommand):
    ...
    def create_profile(self, username, firstname, lastname, email, password) -> Profile:
        ...
        user = User.objects.create(
            username=username, firstname=firstname, lastname=lastname, email=email, password=users_service.hash_password(password)
        )
    ...
    def setup_query_executor_user_account(self) -> None:
        _ = self.create_profile(
            username=QUERY_EXECUTOR_USERNAME,
            firstname="Query",
            lastname="EXECUTOR",
    ...
    def setup_admin_user_account(self) -> None:
        profile = self.create_profile(
            username=ADMIN_USERNAME, firstname=ADMIN_FIRSTNAME, lastname=ADMIN_LASTNAME, email=ADMIN_EMAIL, password=ADMIN_PASSWORD
        )
