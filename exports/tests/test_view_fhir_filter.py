...
class FhirFilterViewSetTests(TestCase):
    def setUp(self):
        ...
        self.filter1 = FhirFilter.objects.create(
            owner=self.user, name="Test1", fhir_resource="Resource1", filter='{"gender": "male"}', auto_generated=False, identifying=False
        )
        self.filter2 = FhirFilter.objects.create(
            owner=self.user, name="Test2", fhir_resource="Resource2", filter='{"code": "1234"}', auto_generated=False, identifying=True
        )
        self.filter3 = FhirFilter.objects.create(
            owner=self.user, name="Test3", fhir_resource="Resource3", filter='{"some-field": "value"}', auto_generated=False, identifying=False
        )
    ...
    def test_list_identifying_filters(self):
        ...
        filters = FhirFilter.objects.filter(owner=self.user, auto_generated=False, identifying=True)
    ...
    def test_list_non_identifying_filters(self):
        ...
        filters = FhirFilter.objects.filter(owner=self.user, auto_generated=False, identifying=False)
        serializer = FhirFilterSerializer(filters, many=True)
        self.assertEqual(response.json().get("results"), serializer.data)
