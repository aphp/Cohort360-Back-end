...
class DatalabViewSetTestBase(ExportsTestBase):
    ...
    def setUp(self):
        super().setUp()
        self.view_root = "exports:v1:datalabs"
    ...
    def test_retrieve_datalab(self):
        retrieve_url = reverse(viewname=f'{self.view_root}-detail', args=[self.datalabs[0].uuid])
    ...
    def test_patch_datalab(self):
        ...
        patch_url = reverse(viewname=f'{self.view_root}-detail', args=[self.target_datalab_to_patch.uuid])
    ...
    def test_delete_datalab(self):
        delete_url = reverse(viewname=f'{self.view_root}-detail', args=[self.target_datalab_to_delete.uuid])
