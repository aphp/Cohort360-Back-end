...
class DatalabViewSetTest(ExportsTestBase):
    ...
    def setUp(self):
        super().setUp()
        self.datalabs = [Datalab.objects.create(name=f"datalab_{i}", infrastructure_provider=self.infra_provider_aphp) for i in range(5)]
    ...
    def test_list_datalabs(self):
        ...
        self.check_test_list_view(
            list_url=list_url, request_user=self.datalabs_reader_user, expected_resp_status=status.HTTP_200_OK, result_count=len(self.datalabs) - 1
        )
    ...
    def test_error_create_datalab_with_no_right(self):
        create_url = reverse(viewname=self.viewname_list)
        self.check_test_create_view(
            request_user=self.user_without_rights,
            create_url=create_url,
