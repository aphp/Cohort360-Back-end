...
class InfrastructureProviderViewSetTestBase(ExportsTestBase):
    view_set = InfrastructureProviderViewSet
    model = InfrastructureProvider

    def setUp(self):
        super().setUp()
        self.view_root = "exports:v1:infrastructure_providers"
    ...
    def test_list_infrastructure_provider(self):
        list_url = reverse(viewname=f'{self.view_root}-list')
    ...
    def test_retrieve_infrastructure_provider(self):
        retrieve_url = reverse(viewname=f'{self.view_root}-detail', args=[self.target_infra_provider_to_retrieve.uuid])
    ...
    def test_create_infrastructure_provider(self):
        create_url = reverse(viewname=f'{self.view_root}-list')
    ...
    def test_patch_infrastructure_provider(self):
        patch_url = reverse(viewname=f'{self.view_root}-detail', args=[self.target_infra_provider_to_patch.uuid])
    ...
    def test_delete_infrastructure_provider(self):
        delete_url = reverse(viewname=f'{self.view_root}-detail', args=[self.target_infra_provider_to_delete.uuid])
