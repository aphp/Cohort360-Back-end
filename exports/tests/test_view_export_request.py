...
def new_cohort_result(
    ...
    if not req:
        req: Request = Request.objects.create(owner=owner, parent_folder=folder, name=random_str(5))
    if not rqs:
        rqs: RequestQuerySnapshot = RequestQuerySnapshot.objects.create(owner=owner, request=req, perimeters_ids=[str(perimeter.id)])
    ...
    cr: CohortResult = CohortResult.objects.create(
        owner=owner, group_id=str(random.randint(0, 10000)), dated_measure=dm, request_query_snapshot=rqs, request_job_status=status
    )
...
class ExportsTests(ViewSetTestsWithBasicPerims):
    ...
    table_unsettable_fields = ["source_table_name", "export_request", "export_request_table_id"]
    ...
    def setUp(self):
        ...
        # ROLES
        (self.role_read_pseudo, self.role_read_nomi, self.role_exp_csv_xlsx_nomi, self.role_exp_jup_pseudo, self.role_exp_jup_nomi) = [
        ...
        # COHORTS
        _, _, _, _, self.user1_cohort = new_cohort_result(owner=self.user1, status=JobStatus.finished.value)

        _, _, _, _, self.user2_cohort = new_cohort_result(owner=self.user2, status=JobStatus.finished.value)
        self.export_type = ExportsConfig.ExportTypes.default()

    def check_is_created(self, base_instance: Export, request_model: dict = None, user: User = None):
        if user is None:
        ...
        for table in request_model.get("export_tables", []):
            ert = ExportTable.objects.filter(name=table.get("name", ""), export=base_instance).first()
            self.assertIsNotNone(ert)

            [
                self.assertNotEqual(getattr(ert, f), table.get(f), f"Error with model's table's {f}")
                for f in self.table_unsettable_fields
                if f in table
            ]

    def check_list_reqs(self, found: List[Export], to_find: List[Export], key: str = ""):
        msg = f"{key} What was to find: \n {[r for r in to_find]}.\n What was found: \n {[r for r in found]}"
        self.assertEqual(len(to_find), len(found), msg)
        found_ids = [er.get("export_request_id", "") for er in found]
        [self.assertIn(er.id, found_ids, f"{msg}.\nMissing request is {er}") for er in to_find]
...
class ExportsWithSimpleSetUp(ExportsTests):
    def setUp(self):
        ...
        self.user1_exp_req_succ: Export = Export.objects.create(
            owner=self.user1,
        ...
        self.user1_exp_req_succ_table1: ExportTable = ExportTable.objects.create(export=self.user1_exp_req_succ, name="table_x")

        self.user1_exp_req_fail: Export = Export.objects.create(
            owner=self.user1,
            output_format=self.export_type,
        ...
        self.user2_exp_req_succ: Export = Export.objects.create(
            owner=self.user2,
            output_format=self.export_type,
        ...
        self.user1_export = Export.objects.create(
...
class ExportsListTests(ExportsTests):
    def setUp(self):
        ...
        _, _, _, _, self.user1_cohort2 = new_cohort_result(self.user1, JobStatus.finished)
        f, r, rqs, dm, self.user2_cohort_1 = new_cohort_result(owner=self.user2, status=JobStatus.finished)
        _, _, _, _, self.user2_cohort2 = new_cohort_result(self.user2, JobStatus.finished, f, r, rqs, dm)
        ...
        self.user1_reqs = Export.objects.bulk_create(
            [
                Export(
                    ...
                    output_format=random.choice([self.export_type, self.export_type]),
                    request_job_status=random.choice([JobStatus.new.value, JobStatus.failed.value, JobStatus.validated.value]),
        ...
        self.user2_reqs: List[Export] = Export.objects.bulk_create(
            [
                Export(
                    output_format=random.choice([self.export_type, self.export_type]),
                    owner=self.user2,
    ...
    def test_list_requests_as_user1(self):
        ...
        self.check_get_paged_list_case(
            ListCase(to_find=base_results, page_size=20, user=self.user1, status=status.HTTP_200_OK, success=True, params={"owner": self.user1.pk})
        )
...
class ExportsNotAllowedTests(ExportsWithSimpleSetUp):
    def setUp(self):
        ...
        role_full: Role = Role.objects.create(**dict([(f, True) for f in self.all_rights]), name="FULL")
        ...
        self.basic_csv_data = {**self.basic_jup_data, "output_format": self.export_type}
...
class ExportsUpdateNotAllowedTests(ExportsNotAllowedTests):
    ...
    def test_error_update_request(self):
        [
            self.check_patch_case(
                PatchCase(user=self.full_admin_user, status=status.HTTP_405_METHOD_NOT_ALLOWED, success=False, initial_data=data, data_to_update={})
            )
            for data in [self.basic_csv_data, self.basic_jup_data]
        ]
