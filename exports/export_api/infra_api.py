...
class InfraAPI:
    ...
    def create_db(self, name: str, location: str) -> str:
        ...
        response = self.post_hadoop(target_endpoint="hive/create_base_hive", params=params)
        return response.json().get('task_id')
    ...
    def change_db_ownership(self, location: str, db_user: str) -> None:
        ...
        response = self.post_hadoop(target_endpoint="hdfs/chown_directory", params=params)
        self.check_response(response=response)

    def post_hadoop(self, target_endpoint: str, params: dict) -> Response:
        return requests.post(url=f"{self.url}/hadoop/{target_endpoint}",
                             params=params,
