...
from exports.tasks import clean_export_requests
...
class ExportsJobsTests(ExportsWithSimpleSetUp):
    ...
    def test_task_clean_export_requests(self, mock_delete_hdfs_file, mock_send_mail):
        from admin_cohort.settings import DAYS_TO_CLEAN_REQUESTS

        mock_delete_hdfs_file.return_value = None
        mock_send_mail.return_value = None

        self.user1_exp_req_succ.review_request_datetime = (
                timezone.now() - timedelta(days=DAYS_TO_CLEAN_REQUESTS))
        self.user1_exp_req_succ.is_user_notified = True
        self.user1_exp_req_succ.save()

        clean_export_requests()
